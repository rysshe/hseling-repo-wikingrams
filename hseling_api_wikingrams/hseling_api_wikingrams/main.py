...
from hseling_api_wikingrams import boilerplate

from hseling_lib_wikingrams.process import process_data
from hseling_lib_wikingrams.query import query_data


app = Flask(__name__)
app.config['DEBUG'] = False
app.config['LOG_DIR'] = '/tmp/'
app.config.from_envvar('HSELING_API_WIKINGRAMS_SETTINGS')
...
if not app.debug:
    ...
    file_handler = TimedRotatingFileHandler(os.path.join(app.config['LOG_DIR'], 'hseling_api_wikingrams.log'), 'midnight')
...
def healthz():
    ...
    return jsonify({"status": "ok", "message": "Application WikiNGrams"})
